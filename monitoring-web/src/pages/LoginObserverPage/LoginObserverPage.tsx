...
import { ObserverCredentials } from '@models';

import { loginObserver } from '@store/auth/auth.action.creators';
...
export const LoginObserverPage = () => {
  const {
    handleSubmit,
    register,
    formState: { errors },
  } = useForm<ObserverCredentials>({
    defaultValues: {
      email: '',
      password: '',
    },
  ...
  const onSubmit = (data: ObserverCredentials) => {
    dispatch(loginObserver(data));
  };
    ...
    <Box sx={{ height: '100%' }}>
      <Box sx={{ px: 3, py: 2, height: '100%', display: 'grid', placeItems: 'center' }}>
        <Paper sx={{ maxWidth: '550px', width: '100%', px: 3, py: 2 }} elevation={5}>
          <Box sx={{ mb: 2 }}>
            <Typography fontSize={24}>Ввійти як наглядач</Typography>
          </Box>
